...
void defaultd(GtkWidget *calculate, gpointer data) {
  system("sh ./tools/defaultmgr.sh && ./tools/Reboot"); // Run default GPU
                                                        // driver script
}
...
static void check_state1(GtkWidget *widget, gpointer data, int argc,
                         char **argv) {
  ...
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget))) { // write to
                                                                 // file
    char cmd[50];
...
static void check_state2(GtkWidget *widget, gpointer data, int argc,
                         char **argv) {
  ifstream file("./data.json"); // Get the state Json data
  Json::Value son;
  Json::Reader reader;
...
static void check_state3(GtkWidget *widget, gpointer data, int argc,
                         char **argv) {
  ifstream file("./data.json"); // Get the state Json data
  Json::Value son;
  Json::Reader reader;
...
static void check_state4(GtkWidget *widget, gpointer data, int argc,
                         char **argv) {
  ifstream file("./data.json"); // Get the state Json data
  Json::Value son;
  Json::Reader reader;
...
static void check_state5(GtkWidget *widget, gpointer data, int argc,
                         char **argv) {
  ifstream file("./data.json"); // Get the state Json data
  Json::Value son;
  Json::Reader reader;
...
static void check_state6(GtkWidget *widget, gpointer data, int argc,
                         char **argv) {
  ifstream file("./data.json"); // Get the state Json data
  Json::Value son;
  Json::Reader reader;
...
static void check_state7(GtkWidget *widget, gpointer data, int argc,
                         char **argv) {
  ifstream file("./data.json"); // Get the state Json data
  Json::Value son;
  Json::Reader reader;
...
static void check_state8(GtkWidget *widget, gpointer data, int argc,
                         char **argv) {
  ifstream file("./data.json"); // Get the state Json data
  Json::Value son;
  Json::Reader reader;
...
static void check_state9(GtkWidget *widget, gpointer data, int argc,
                         char **argv) {
  ifstream file("./data.json"); // Get the state Json data
  Json::Value son;
  Json::Reader reader;
...
static void check_state10(GtkWidget *widget, gpointer data, int argc,
                          char **argv) {
  ifstream file("./data.json"); // Get the state Json data
  Json::Value son;
  Json::Reader reader;
...
static void check_state11(GtkWidget *widget, gpointer data, int argc,
                          char **argv) {
  ifstream file("./data.json"); // Get the state Json data
  Json::Value son;
  Json::Reader reader;
...
static void check_state12(GtkWidget *widget, gpointer data, int argc,
                          char **argv) {
  ifstream file("./data.json"); // Get the state Json data
  Json::Value son;
  Json::Reader reader;
...
static void check_state13(GtkWidget *widget, gpointer data, int argc,
                          char **argv) {
  ifstream file("./data.json"); // Get the state Json data
  Json::Value son;
  Json::Reader reader;
...
static void check_state14(GtkWidget *widget, gpointer data, int argc,
                          char **argv) {
  ifstream file("./data.json"); // Get the state Json data
  Json::Value son;
  Json::Reader reader;
...
static void check_state15(GtkWidget *widget, gpointer data, int argc,
                          char **argv) {
  ifstream file("./data.json"); // Get the state Json data
  Json::Value son;
  Json::Reader reader;
  ...
  if (geteuid() == 0) {
    if (access("./data.json", F_OK) != -1) {
      ...
      gtk_label_set_markup(
          GTK_LABEL(label),
          "<span font='18'><b>Welcome to GPU Passthrough Manager\n</b></span>");
      gtk_container_add(GTK_CONTAINER(vbox), label);
      gtk_box_pack_start(GTK_BOX(vbox), label, 0, 1, 1);
      gtk_label_set_justify(GTK_LABEL(label), GTK_JUSTIFY_CENTER);
      // info
      para = gtk_label_new_with_mnemonic("null");
      gtk_label_set_markup(GTK_LABEL(para),
                           "<span font='12'>Select devices to load VFIO "
                           "drivers, or load default drivers</span>");
      ...
      for (int i = 0; i < actualJson["devices"].size(); i++) {
        GtkWidget *toggle[i];
        toggle[i] = gtk_toggle_button_new_with_label(
            actualJson["devices"][i]["name"].asString().c_str());
        gtk_box_pack_start(GTK_BOX(vbox), toggle[i], 0, 1, 1);
        // buttons callback to individual functions... I know, this is very
        // unoptimised. I am ashamed.
        switch (i) {
        case 0:
          g_signal_connect(toggle[i], "toggled", G_CALLBACK(check_state1),
                           NULL);
          break;
        case 1:
          g_signal_connect(toggle[i], "toggled", G_CALLBACK(check_state2),
                           NULL);
          break;
        case 2:
          g_signal_connect(toggle[i], "toggled", G_CALLBACK(check_state3),
                           NULL);
          break;
        case 3:
          g_signal_connect(toggle[i], "toggled", G_CALLBACK(check_state4),
                           NULL);
          break;
        case 4:
          g_signal_connect(toggle[i], "toggled", G_CALLBACK(check_state5),
                           NULL);
          break;
        case 5:
          g_signal_connect(toggle[i], "toggled", G_CALLBACK(check_state6),
                           NULL);
          break;
        case 6:
          g_signal_connect(toggle[i], "toggled", G_CALLBACK(check_state7),
                           NULL);
          break;
        case 7:
          g_signal_connect(toggle[i], "toggled", G_CALLBACK(check_state8),
                           NULL);
          break;
        case 8:
          g_signal_connect(toggle[i], "toggled", G_CALLBACK(check_state9),
                           NULL);
          break;
        case 9:
          g_signal_connect(toggle[i], "toggled", G_CALLBACK(check_state10),
                           NULL);
          break;
        case 10:
          g_signal_connect(toggle[i], "toggled", G_CALLBACK(check_state11),
                           NULL);
          break;
        case 11:
          g_signal_connect(toggle[i], "toggled", G_CALLBACK(check_state12),
                           NULL);
          break;
        case 12:
          g_signal_connect(toggle[i], "toggled", G_CALLBACK(check_state13),
                           NULL);
          break;
        case 13:
          g_signal_connect(toggle[i], "toggled", G_CALLBACK(check_state14),
                           NULL);
          break;
        case 14:
          g_signal_connect(toggle[i], "toggled", G_CALLBACK(check_state15),
                           NULL);
          break;
        }
      ...
      gtk_label_set_markup(
          GTK_LABEL(para),
          "<span font='12'>This program is used for assisting in passing "
          "through\ngraphics/audio devices to virtual machines.\nTo begin, "
          "click the button below to run initial\ncompatibility checks and "
          "configuration adjustments.\nBefore continuing, ensure your system "
          "supports IOMMU\nand virtualization is enabled in BIOS. Installation "
          "will \nfail if these requirements are not met and your\noperating "
          "system may become unstable and/or unusable.\n\n</span>");
